...
class TestStlTo3mf(TestCase):
    ...
    def assertZipEqual(self, zip_data1, zip_path2, msg=None):
        ...
        with ZipFile(zip_data1, 'r') as zip1, \
             ZipFile(zip_path2, 'r') as zip2:
            ...
            self.assertEqual(
                files1, 
                files2, 
                f"{msg_prefix}ZIP files contain different files")
            
            # Compare contents of each file
            for filename in files1:
                with zip1.open(filename) as f1, \
                     zip2.open(filename) as f2:
                    content1 = f1.read()
                    content2 = f2.read()
                    self.assertEqual(
                        content1,
                        content2,
                        f"{msg_prefix}Content mismatch in {filename}")
